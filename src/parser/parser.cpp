...
std::string Parser::processCommand(const std::vector<std::string> &tokens)
{
    ...
    {
        if (tokens.size() < 3)
            return "-ERR SETNX requires key value";
    ...
    {
        if (tokens.size() < 3)
            return "-ERR APPEND requires key value";
    ...
    {
        if (tokens.size() < 3)
            return "-ERR INCRBY requires key amount";
    ...
    {
        if (tokens.size() < 3)
            return "-ERR DECRBY requires key amount";
    ...
    {
        if (tokens.size() < 3)
            return "-ERR LPUSH requires list value";
    ...
    {
        if (tokens.size() < 3)
            return "-ERR RPUSH requires list value";
    ...
    {
        if (tokens.size() < 3)
            return "-ERR LINDEX requires list and index";
    ...
    {
        if (tokens.size() < 4)
            return "-ERR LSET requires list, index, and value";
    ...
    {
        if (tokens.size() < 3)
            return "-ERR LSORT requires list and order";
    ...
    {
        if (tokens.size() < 2)
            return "-ERR LPRINT requires list";
    ...
    {
        if (tokens.size() < 3)
            return "-ERR SADD requires set value";
    ...
    {
        if (tokens.size() < 3)
            return "-ERR SREM requires set value";
    ...
    {
        if (tokens.size() < 2)
            return "-ERR SMEMBERS requires set";
    ...
    {
        if (tokens.size() < 3)
            return "-ERR SISMEMBER requires set value";
    ...
    {
        if (tokens.size() < 3)
            return "-ERR SUNION requires two sets";
    ...
    {
        if (tokens.size() < 3)
            return "-ERR SINTER requires two sets";
    ...
    {
        if (tokens.size() < 3)
            return "-ERR SDIFF requires two sets";
    ...
    {
        if (tokens.size() < 4)
            return "-ERR HSET requires key field value";
    ...
    {
        if (tokens.size() < 3)
            return "-ERR HGET requires key field";
    ...
    {
        if (tokens.size() < 3)
            return "-ERR HDEL requires key field(s)";
        std::vector<std::string> fields(tokens.begin() + 2, tokens.end());
        return hashmapstore::hdel(baseMap, tokens[1], fields);
    }

    if (cmd == "HEXISTS")
    {
        if (tokens.size() < 3)
            return "-ERR HEXISTS requires key field";
    ...
    // expiry and the ttl commands
    // else if (cmd == "EXPIRE")
    // {
    //     std::string key = tokens[1];
    //     int seconds = std::stoi(tokens[2]);
    //     bool ok = baseMap.expire(key, seconds);
    //     return ok ? "1" : "0";
    // }
    // else if (cmd == "TTL")
    // {
    //     std::string key = tokens[1];
    //     long ttl = baseMap.ttl(key);
